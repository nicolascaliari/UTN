...
int jugador_imprimirArray(eJugador arrayJugador[], int limiteJugador, eConfederacion arrayConfederacion[], int limiteConfederacion) {
	...
	if (arrayJugador != NULL && limiteJugador > 0) {
		...
		printf("----------------------------------------------------------------------------------------------------------------------\n");
		for (i = 0; i < limiteJugador; i++) {
			jugador_imprimir(arrayJugador[i], arrayConfederacion,
					limiteConfederacion);
	...
	if (arrayJugador != NULL && limiteJugador > 0) {
		...
		if (indice != -1) {
			...
			if (utn_getDescripcion(auxiliar.nombre, 50, "\nIngrese su nombre\n",
					"\nERROR\n", 2) == 0
					&& utn_getDescripcion(auxiliar.posicion, 50,
							"Ingrese su posicion", "error", 2) == 0
					&& utn_getNumeroShort(&auxiliar.numeroCamiseta,
							"Ingrese su numero de camiseta", "error", 1, 90, 2)
							== 0
					&& utn_getNumeroFlotante(&auxiliar.salario,
							"Ingrese su salario", "error", 1, 600000, 2) == 0
					&& utn_getNumeroShort(&auxiliar.aniosContrato,
							"Ingrese anios de contrato", "error", 1, 50, 2)
							== 0)

							{
				auxiliar.idConfederacion = pedirConfederacion(arrayConferacion,
						limiteConfederacion);
			}
			auxiliar.isEmpty = 1;
...
//		jugador_imprimirArray(arrayJugador, limiteJugador);
		if (utn_getNumero(&id, "Ingrese el ID que desea dar de baja",
				"Error al ingresar id", 1, 20, 10000) == 0) {

			while (recurso_buscarId(arrayJugador, limiteJugador, id) == -1) {
				puts("NO EXISTE ID.");
				utn_getNumero(&id, "Ingrese el ID que desea dar de baja",
